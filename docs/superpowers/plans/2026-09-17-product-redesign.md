...
Deployment: test the application image on isolated data, take a consistent live backup, update the existing local Compose image, verify health and data. Publish an immutable static release on Contabo, retain previous release, push the private repository and record status.
