...
Amfora runs in Docker, with SQLite and bundled storage on a persistent mount or external S3. Startup applies schema changes. CI validates the application, and the release workflow publishes versioned container images. The public source installer builds locally. An application updater and signed update manifest are not implemented.
...
1. Publish tested versioned images and a signed release manifest containing an immutable image digest, compatibility information and release notes. If a deployment uses a registry that requires authentication, keep its credentials on the host.
...
The website now leads with installation, and GitHub has a branded README banner. This document replaces the earlier backup based proposal. No updater or automatic update schedule is enabled. Versioned container images are now published through the release workflow.
